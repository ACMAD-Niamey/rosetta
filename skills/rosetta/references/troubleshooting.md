...
- Cache key: `(product, variable, date_range, region-bbox, init_months, init_date, target_months)` under namespace `rosetta`, versioned by `_CACHE_VERSION` (currently 8). A version bump invalidates all cached entries. Polygon geometries never enter the key.
